...
Generates deterministic (replicable) seed data:
  ...
  - Syllabus entries (Programme × Course) — Approved for core/common units,
    Adjunct for electives — required before any Curriculum can reference
    that course under that programme
  - Curriculum records per class per session, pointing at a Syllabus entry
    rather than a Course directly (same year-of-study = same courses)
...
NOTE ON RegistrationWindow:
  Enrollment.clean() now requires an open RegistrationWindow for the
  relevant session/programme before an enrollment can be saved normally.
  This command still uses Enrollment.objects.bulk_create(...), which
  bypasses .save()/.clean() entirely, so seeding itself is unaffected.
  BUT: if you (or the UI) later try to manually create/approve an
  Enrollment against this seed data via .save(), it will fail until a
  RegistrationWindow row exists for that session. This command does not
  create RegistrationWindow rows because its exact field set wasn't
  available when this was written — add a small `_create_registration_windows`
  step once you can confirm its fields (term, window_type, programme,
  is_open/opens_at/closes_at, etc.).
...
NOTE ON Curriculum.clone_curriculum():
  The classmethod on the model still references `req.course`,
  `req.course_id`, and constructs new Curriculum(course=...) — all of
  which broke when `course` became a property instead of a field. This
  command doesn't call clone_curriculum(), so it isn't a blocker here,
  but it will raise AttributeError if invoked as-is. Flagging since it's
  part of the same model file.
"""
...
from base.models import (
    ...
    Syllabus, Curriculum, LecturerAssignment, Result, FeeStructure,
    StudentFeeAccount, Payment, Hostel, Room, HostelAllocation,
...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # needs to be satisfiable before any Curriculum can be built.
        ...
        for school_data in SCHOOL_DATA:
            ...
            for dept_data in school_data["departments"]:
                ...
                for prog_data in dept_data["programmes"]:
                    ...
                    #    used by this programme, BEFORE any Curriculum can
                    #    reference it. Core/Common units go straight to
                    #    Approved; Electives go to Adjunct — both are
    ...
    def _assign_lecturers_to_curricula(self, lecturers, sessions):
        ...
        # `course` is now a property (Curriculum.syllabus.course), not a
        # real FK field, so the select_related path has to traverse
        # through syllabus explicitly. curr.course.department_id below
        # still works fine post-fetch — the property just delegates.
        curricula = Curriculum.objects.select_related(
            "syllabus__course__department"
        ).all()

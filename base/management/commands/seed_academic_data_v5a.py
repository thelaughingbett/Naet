...
Generates deterministic (replicable) seed data:
  ...
  - Prints the full current hostel-resident list (new this run AND
    pre-existing from earlier runs — see the note below)

NOTE ON HOSTEL RESIDENT REPORTING:
  `_maybe_allocate_hostel` used to only return 1/0 for whether a *new*
  HostelAllocation row was created this run, which meant the end-of-run
  summary silently showed "0 hostel allocations" on reruns even though
  residents from a prior run obviously still exist. It now also appends
  every resident it touches — new OR pre-existing — to
  self._hostel_residents, and `_print_hostel_residents()` prints that
  full list at the end of the run, each row tagged [NEW] or [existing].
  The 0/1 return value is kept as-is (still used for the aggregate
  "created this run" count in the class-loop summary line).
...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._hostel_residents = []  # every resident touched this run, new OR pre-existing
    ...
    def _maybe_allocate_hostel(self, student, active_session, rooms_by_gender):
        ...
        used purely for the "created this run" tally in the class-loop
        summary. Separately, every student who ends up resident — whether
        the HostelAllocation row was newly created here or already existed
        from a previous run — is appended to self._hostel_residents, so
        the end-of-run report (_print_hostel_residents) always reflects
        the full current resident list, not just this run's deltas.
        """
        ...
        # Record this resident regardless of created/pre-existing, using
        # the allocation's actual room (in case it already existed from a
        # prior run and was assigned a different room than `room` above).
        self._hostel_residents.append({
    ...
    def _create_students_and_results(self, sessions, hostel_rooms_by_gender):
        ...
        self.stdout.write(
            ...
            f"{total_hostel_allocations} hostel allocations created this run."
        )
    ...
    # ── print hostel residents ──────────────────────────────────────────────

    def _print_hostel_residents(self):
        """
        Prints the FULL current set of hostel residents this run touched —
        both newly-created allocations and ones that already existed from
        an earlier run — each tagged [NEW]/[existing]. Unlike the
        "N hostel allocations created this run" line in the students
        summary (which only counts new rows and silently reads 0 on a
        rerun), this always shows who's actually resident.
        """
        self.stdout.write(self.style.MIGRATE_HEADING(
            "\n══════════════════════════════════════════════════════"
        ))
        ...
        if not self._hostel_residents:
            self.stdout.write("  (no hostel residents)")
            return

        new_count = sum(
            1 for r in self._hostel_residents if r["new_this_run"])
        self.stdout.write(
            f"\n  {len(self._hostel_residents)} resident(s) total "
            f"({new_count} new this run, "
            f"{len(self._hostel_residents) - new_count} pre-existing):"
        )
        for r in self._hostel_residents:
            marker = "NEW" if r["new_this_run"] else "existing"
            self.stdout.write(
                f"    [{marker:>8}] {r['registration_number']:<16} {r['name']:<25} "
                f"{r['email']:<40} ({r['gender']})  →  "
                f"{r['hostel']} / Room {r['room']}  [{r['session']}]"
            )

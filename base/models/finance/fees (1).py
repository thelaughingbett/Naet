...
class FeeCategory(BaseModelMixin):
    """
    A billable fee line item type — tuition, registration, library, sports,
    caution money, etc. Shared reference data so FeeStructureItem.category,
    Scholarship.covered_category, and Charge can all FK against the same
    validated set rather than matching free-text strings against an
    untyped JSON blob (the old FeeStructure.breakdown design this
    replaces — see fees.py's module history for why).
    """
...
class FeeStructure(BaseModelMixin):
    """Defines what a class owes per session — the itemised breakdown
    lives in FeeStructureItem, not inline here."""
    ...
    def total_amount(self):
        """
        Unchanged interface from the old JSON-backed version — every
        caller (StudentFeeAccount.amount_billed, etc.) keeps working
        without modification. Only the source of truth changed: a real
        aggregate over FeeStructureItem rows instead of summing a dict
        in Python.
        """
        total = self.items.aggregate(total=models.Sum('amount'))['total']
...
class FeeStructureItem(BaseModelMixin):
    """One category's billed amount within a FeeStructure — e.g. 'tuition:
    45000'. Replaces what used to be a key in FeeStructure.breakdown's
    ...
    class Meta:
        ...
        ordering = ['fee_structure', 'category']
        verbose_name = "Fee Structure Item"
        verbose_name_plural = "Fee Structure Items"

    def __str__(self):
        return f"{self.fee_structure} — {self.category}: KES {self.amount}"
...
class Sponsor(BaseModelMixin):
    ...
    sponsor_type = models.CharField(
        max_length=20, choices=SPONSOR_TYPE_CHOICES, default='other'
    )
...
class Scholarship(BaseModelMixin):
    ...
    sponsor = models.ForeignKey(
        'Sponsor', on_delete=models.PROTECT, related_name='scholarships'
    )

    name = models.CharField(max_length=200)
    coverage_type = models.CharField(
        max_length=20, choices=COVERAGE_TYPE_CHOICES
    )
    ...
    def clean(self):
        ...
        if self.coverage_type == 'category_specific' and not self.covered_category_id:
            raise ValidationError({
    ...
    def compute_award_amount(self, account):
        ...
        if self.coverage_type == 'category_specific':
            item = account.fee_structure.items.filter(
                category=self.covered_category
            ).first()
            category_amount = item.amount if item else Decimal('0.00')
...
class ScholarshipAward(BaseModelMixin):
    ...
    student = models.ForeignKey(
        'Student', on_delete=models.CASCADE, related_name='scholarship_awards'
    )
    account = models.ForeignKey(
        'StudentFeeAccount', on_delete=models.PROTECT,
        related_name='scholarship_awards',
        help_text="The fee account this award applies against.",
    )

    amount_covered = models.DecimalField(max_digits=10, decimal_places=2)
...
class LateFeePolicy(BaseModelMixin):
    ...
    fee_structure = models.OneToOneField(
        'FeeStructure', on_delete=models.CASCADE, related_name='late_fee_policy'
    )
    ...
    penalty_type = models.CharField(
        max_length=12, choices=PENALTY_TYPE_CHOICES, default='fixed'
    )

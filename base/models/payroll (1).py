...
class StatutoryRate(BaseModelMixin):
    ...
    deduction_type = models.ForeignKey(
        'StatutoryDeductionType', on_delete=models.PROTECT, related_name='rates'
    )
    ...
    def compute_employee_amount(self, gross_pay):
        base = min(gross_pay, self.maximum_pensionable_pay) if self.maximum_pensionable_pay else gross_pay
        amount = (base * self.employee_percentage / Decimal('100')).quantize(Decimal('0.01'))
...
class StaffCompensation(BaseModelMixin):
    ...
    staff = models.ForeignKey(
        'User', on_delete=models.CASCADE, related_name='compensation_history',
    ...
    cost_centre = models.ForeignKey(
        ...
        related_name='staff_compensation_records',
        help_text="Optional — which CostCentre's Personnel BudgetLine "
                   "(budgeting.py) this staff member's pay counts against.",
    )
...
class SalaryAllowance(BaseModelMixin):
    ...
    staff = models.ForeignKey(
        'User', on_delete=models.CASCADE, related_name='salary_allowances',
        limit_choices_to={'is_staff': True},
    )
...
class StaffLoan(BaseModelMixin):
    ...
    staff = models.ForeignKey(
        'User', on_delete=models.PROTECT, related_name='staff_loans',
        limit_choices_to={'is_staff': True},
    )
...
class StaffDeductionMandate(BaseModelMixin):
    ...
    staff = models.ForeignKey(
        'User', on_delete=models.CASCADE, related_name='deduction_mandates',
        limit_choices_to={'is_staff': True},
    )
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES, default='other')
    ...
    loan = models.ForeignKey(
        'StaffLoan', on_delete=models.SET_NULL, null=True, blank=True,
...
class PayrollRun(BaseModelMixin):
    ...
    def total_employer_cost(self):
        total = self.payslips.aggregate(total=models.Sum('total_employer_cost'))['total']
        return total or Decimal('0.00')
    ...
    def compute(self, staff_queryset=None, as_of=None):
        ...
        if self.status not in ('draft', 'computed'):
            raise ValidationError("Can only compute a draft or already-computed run.")

        from django.contrib.auth import get_user_model
        User = get_user_model()
        as_of = as_of or self.period.pay_date
        staff_queryset = staff_queryset or User.objects.filter(is_staff=True, is_active=True)
...
class Payslip(BaseModelMixin):
    ...
    compensation = models.ForeignKey(
        'StaffCompensation', on_delete=models.PROTECT, related_name='payslips'
    )

    gross_pay = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))
    taxable_pay = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))
    total_statutory_deductions = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))
    total_voluntary_deductions = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))
    ...
    def calculate(self, as_of=None):
        ...
        if paye > 0:
            paye_type = StatutoryDeductionType.objects.filter(code='paye').first()
            PayslipLine.objects.create(
...
class PayslipLine(BaseModelMixin):
    ...
    statutory_deduction_type = models.ForeignKey(
        'StatutoryDeductionType', on_delete=models.SET_NULL, null=True, blank=True,
        related_name='payslip_lines',
    )
    deduction_mandate = models.ForeignKey(
        'StaffDeductionMandate', on_delete=models.SET_NULL, null=True, blank=True,
        related_name='payslip_lines',
    )
...
class StatutoryRemittance(BaseModelMixin):
    ...
    period = models.ForeignKey(
        'PayrollPeriod', on_delete=models.PROTECT, related_name='remittances'
    )
    deduction_type = models.ForeignKey(
        'StatutoryDeductionType', on_delete=models.PROTECT, related_name='remittances'
    )
